...
export const Route = createFileRoute("/buscar")({
  ...
  head: () => ({ meta: [{ title: "Busca — Direct Ofertas" }] }),
      ...
      <main className="flex-1 mx-auto max-w-7xl w-full px-4 py-6">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          <h1 className="text-xl font-bold">
            {q ? <>Resultados para "<span className="text-primary">{q}</span>"</> : "Todos os produtos"}
            {items && <span className="text-sm text-muted-foreground font-normal ml-2">({items.length})</span>}
          </h1>
          <select
            value={sort}
